...
import { discoverWithFilters, getImageUrl } from '../lib/tmdb';
...
export default function ProviderHub() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        document.title = `${name || 'Streaming Provider'} - NotFlix`;
        setLoading(false);
      } catch (error) {
        console.error('Error loading provider hub:', error);
        if (mounted) {
          setLoading(false);
        }
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-xl">Loading provider...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-8">The requested streaming provider could not be found.</p>
  ...
  const displayName = providerName || `Streaming Provider #${providerId}`;
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="bg-gray-800 rounded-lg p-6 mb-12 flex flex-col md:flex-row gap-6 items-start">
        {providerLogo && (
          <div className="bg-white rounded-lg p-4 inline-block flex-shrink-0">
            <img
              src={getImageUrl(providerLogo, 'w300')}
              alt={displayName}
      ...
      {!hasMovies && !hasTv && (
        <p className="text-gray-400 text-center py-12">
          No titles currently found on this platform (US).
        </p>
      )}

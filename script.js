...
function handleMouseMove(event) {
    ...
    focusArray.forEach(element => {
        if (element === currentFocus) return;

        const rect = element.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;

        const isRightOf = centerX > currentCenterX;
        const isLeftOf = centerX < currentCenterX;
        const isBelow = centerY > currentCenterY;
        const isAbove = centerY < currentCenterY;

        const isMovingRight = mouseX > currentCenterX;
        const isMovingLeft = mouseX < currentCenterX;
        const isMovingDown = mouseY > currentCenterY;
        const isMovingUp = mouseY < currentCenterY;

        if ((isMovingRight && isRightOf) || (isMovingLeft && isLeftOf) || 
            (isMovingDown && isBelow) || (isMovingUp && isAbove)) {
